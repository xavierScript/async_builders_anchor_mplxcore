...
use crate::{error::MPLXCoreError, program::AsyncBuildersAnchorMplxcore, state::WhitelistedCreators};
...
pub struct WhitelistCreator<'info> {
    ...
    // Ensuring the program being called is the current program
    #[account(constraint = this_program.programdata_address()? == Some(program_data.key()))]
    pub this_program: Program<'info, AsyncBuildersAnchorMplxcore>,
